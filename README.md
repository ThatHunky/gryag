...
- **Мультимедіа**: підтримує зображення, відео, голосові повідомлення та стікери — коротко описує їх у відповіді
- **Генерація зображень**: створює зображення за запитом через Gemini 2.5 Flash Image (квота 3/день)
- **Телеграм-шашки**: гра шашки між користувачами з inline-кнопками
- **Text-to-Speech**: озвучує текст українською
- **Адаптивний тротлінг**: спокійні користувачі отримують більший ліміт, спамери стискаються
- **Інструменти та API**: калькулятор, погода, курси валют, опитування, веб-пошук
- **Нагадування про донати**: налаштовувані нагадування для чатів
...
Детальна документація: [HYBRID_EXTRACTION_COMPLETE.md](docs/features/HYBRID_EXTRACTION_COMPLETE.md)
...
## Налаштування, які були в гряга

Як був налаштований бот у продакшні:

- **Модель**: Gemini Flash (latest) з увімкненим режимом «думання» (4096 токенів), але reasoning приховано від користувачів
- **Контекст**: до 200 останніх повідомлень у системному контексті, тільки в межах треду
- **Ліміт**: 15 повідомлень на користувача на годину
- **Пам'ять**: tool-based memory (бот сам вирішує що запам'ятати через function calling)
- **Генерація картинок**: через Pollinations (не Gemini)
- **Веб-пошук**: увімкнено (Google Search Grounding)
- **Профілювання**: бот вивчає користувачів та витягує факти
- **Епізоди**: автоматичне створення епізодів з Gemini-сумаризацією
- **Донати**: нагадування увімкнено (крім ігнорованих чатів)
- **CPU-економія**: вимкнено multi-level context, hybrid search, episodic memory, bot self-learning, continuous monitoring
...
- `/gryagfacts` — список фактів про користувача з пагінацією.
- `/gryagchatfacts` — факти про чат (chat-level memory).
- `/gryagself` — переглянути self-learning профіль бота (ефективність, вивчені факти).
- `/gryaginsights` — згенерувати Gemini-інсайти про те, як бот може покращитися.
- `/gryagprompt` — переглянути активний системний промпт.
- `/gryagsetprompt` — встановити кастомний системний промпт.
- `/gryagresetprompt` — скинути промпт до стандартного.
- `/gryagreset` — скинути rate-ліміти для користувача.
- `/broadcast` — транслювати повідомлення у всі приватні чати (reply to message).
