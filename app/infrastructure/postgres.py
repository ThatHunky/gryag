...
async def get_pool(database_url: str, min_size: int = 10, max_size: int = 20) -> asyncpg.Pool:
...
async def get_db_connection(database_url: str) -> AsyncIterator[asyncpg.Connection]:
    ...
    utilization = (pool_stats["active_connections"] / pool_stats["size"]) * 100 if pool_stats["size"] > 0 else 0
    if utilization > 80:
        logger.warning(
