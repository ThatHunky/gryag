...
class CheckersGame:
    ...
    def _get_moves_from(self, row: int, col: int, player: Player) -> list[Move]:
        ...
        for dr, dc in directions:
            if is_king:
                ...
                if self._is_valid_position(new_row, new_col) and self._is_playable_square(new_row, new_col):
                    if self.board[new_row][new_col] == 0:
                        moves.append(Move(row, col, new_row, new_col, []))
        ...
        for dr, dc in directions:
            if is_king:
                ...
                for distance in range(1, self.BOARD_SIZE):
                    ...
                    if additional_jumps:
                        ...
                        for multi_move in additional_jumps:
                            ...
                            jumps.append(Move(row, col, multi_move.to_row, multi_move.to_col, combined_jumps))
                    else:
                        jumps.append(move)
                ...
                if (self._is_valid_position(jump_row, jump_col) and
                    ...
                    self._is_playable_square(land_row, land_col) and
                    self.board[land_row][land_col] == 0):
                    
                    jumped_piece = self.board[jump_row][jump_col]
                    if jumped_piece != 0 and not self._is_player_piece(jumped_piece, player):
                        # Valid jump found, check for multi-jumps
                        jump_pos = (jump_row, jump_col)
                        move = Move(row, col, land_row, land_col, [jump_pos])
    ...
    def _get_jumps_from_recursive(self, row: int, col: int, player: Player, board: list[list[int]]) -> list[Move]:
        ...
        for dr, dc in directions:
            if is_king:
                ...
                for distance in range(1, self.BOARD_SIZE):
                    ...
                    additional = self._get_jumps_from_recursive(land_row, land_col, player, temp_board)
                    if additional:
                        for multi in additional:
                            combined = [jump_pos] + multi.jumps
                            jumps.append(Move(row, col, multi.to_row, multi.to_col, combined))
                    else:
                        jumps.append(move)
                ...
                if (0 <= jump_row < 8 and 0 <= jump_col < 8 and
                    0 <= land_row < 8 and 0 <= land_col < 8 and
                    (land_row + land_col) % 2 == 1 and
    ...
    def make_move(self, move: Move, player: Player) -> bool:
        ...
        if player == 1 and matching_move.to_row == 7:  # Black reaches bottom
            if piece == 1:
                self.board[matching_move.to_row][matching_move.to_col] = 3  # Promote to king
        elif player == 2 and matching_move.to_row == 0:  # White reaches top
            if piece == 2:
                self.board[matching_move.to_row][matching_move.to_col] = 4  # Promote to king
