...
def sanitize_placeholder_text(text: str) -> str:
    """
    Remove leaked placeholder patterns from text before adding to context.

    Placeholders use Unicode Private Use Area (\ue000, \ue001) as delimiters,
    but if these get stripped by Unicode normalization or encoding issues,
    the bare pattern text (TGFMT1, RAW2, etc.) can leak into context and
    be regenerated by the LLM.

    This function removes:
    - Private Use Area delimiter characters
    - Bare placeholder patterns without delimiters

    Args:
        text: Input text that may contain placeholder artifacts

    Returns:
        Text with all placeholder patterns removed
    """
    if not text:
        return text

    # Remove Unicode Private Use Area characters (placeholder delimiters)
    cleaned = text.replace("\ue000", "").replace("\ue001", "")

    # Remove bare placeholder patterns that lost their delimiters
    # Pattern: TGFMT followed by digits, or RAW followed by digits
    cleaned = re.sub(r"TGFMT\d+|RAW\d+", "", cleaned)
...
def format_message_compact(
    ...
    if reply_excerpt:
        ...
        clean_excerpt = (
            sanitize_placeholder_text(reply_excerpt).replace("\n", " ").strip()
        )
    ...
    if text:
        # Sanitize text to remove any leaked placeholder patterns
        content_parts.append(sanitize_placeholder_text(text))
